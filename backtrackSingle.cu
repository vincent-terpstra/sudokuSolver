...
__global__ void solve(int* d_a) {
	...
	for (int idx = 0; idx < N; ++idx) {
		...
		if (at == UNASSIGNED && !(rowHas[row][num] || boxHas[box][num] || colHas[col][num])) {
			...
			guess = num;
			rowCount[row][num]++;    //how many values this section can have
			colCount[col][num]++;
			boxCount[box][num]++;
		}
		__syncthreads();
	...
	__shared__ bool unsolved;
	int num = offset - 1; //Ensure every thread is working on a seperate value in each section 
	int cycle = 0;        //Limit solver to MAXCYCLES
	do {
		__syncthreads();
			unsolved = false;
		num = (num + 1) % N;
		...
		if(!rowHas[row][col])
			unsolved = true;
		__syncthreads();
	} while (unsolved && ++cycle < MAXCYCLES);

	if (!(rowHas[row][col] && colHas[row][col] && boxHas[box][col]))
		unsolved = true; //HAVE NOT SOLVED the sudoku

	__syncthreads();
	if (unsolved && gridIdx == 0)
		at = 0;
